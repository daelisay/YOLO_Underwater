...
# --- Modul SE Block (Squeeze-and-Excitation) ---
...
class YOLOLayer(nn.Module):
    def __init__(self, anchors, num_classes, img_dim=416):
        ...
        # Gunakan BCEWithLogitsLoss agar loss lebih stabil (menggabungkan sigmoid+BCELoss)
    ...
    def forward(self, x, targets=None, indexes=None, img_dim=None):
        ...
        # Prediksi komponen bbox (logits)
        x_center = torch.sigmoid(prediction[..., 0])  # center x
        y_center = torch.sigmoid(prediction[..., 1])  # center y
        w = prediction[..., 2]  # width (log scale)
        h = prediction[..., 3]  # height (log scale)
        raw_conf = prediction[..., 4]  # logits objectness (belum sigmoid)
        raw_cls = prediction[..., 5:]  # logits class (belum sigmoid)
        ...
        # Output untuk inference: gunakan sigmoid di sini
        output = torch.cat(
            (
        ...
        if targets is None:
            ...
            # Loss fungsi dengan BCEWithLogitsLoss, tanpa perlu sigmoid manual
            ...
            loss_conf = self.obj_scale * loss_conf_obj.mean() + self.noobj_scale * loss_conf_noobj.mean()

            loss_cls = self.bce_loss(raw_cls[obj_mask], tcls[obj_mask]).mean()
