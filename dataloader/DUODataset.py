...
class DUODataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        bbs = BoundingBoxesOnImage([BoundingBox(*box) for box in boxes], shape=img.shape)
        img_aug, boxes_aug = self.transform(image=img, bounding_boxes=bbs)
        boxes_aug = boxes_aug.clip_out_of_image()  # Hanya clip, tidak remove

        # JIKA hasil augmentasi menghapus semua bounding box (karena out-of-image), kita pakai yang asli
        if len(boxes_aug) == 0:
            boxes_aug = [BoundingBox(0, 0, 1, 1)]  # dummy box
            classes = [0]
