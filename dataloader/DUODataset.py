...
class DUODataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root_dir, annotation_file, image_folder='/kaggle/working/YOLO_Underwater/data/image_folder',
        ...
        self.transform = self.get_augmentation(split, use_augmentation)
        self.no_aug_transform = self.get_augmentation(split, False)  # fallback tanpa augmentasi
    ...
    def __getitem__(self, index):
        try:
            ...
            # Jika augmentasi membuang semua box, fallback ke versi tanpa augmentasi
            if len(boxes_aug) == 0:
                print(f"Warning: Augmentation removed all boxes at index {index}, falling back to original image.")
                img_aug, boxes_aug = self.no_aug_transform(image=img, bounding_boxes=bbs)
                boxes_aug = boxes_aug.remove_out_of_image().clip_out_of_image()
                if len(boxes_aug) == 0:
                    raise ValueError("No bounding boxes even after fallback")
